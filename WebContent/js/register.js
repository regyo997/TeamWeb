...
function check_Data()//正則檢驗格式
  {
	...
	pw1 = document.getElementById("mem_pwd").value;
	pw2 = document.getElementById("mem_pwd2").value;
	var re_name=/..+/;
...
function validateIt() {//驗證驗證碼
  ...
  function queryEmail(){//檢查email是否重複註冊　沒有就submit
	...
	try{
		xmlhttp.open("POST","http://localhost:8080/TeamWeb/isEmailAlreadyRegister",true);//指定傳送時的參數　方式、指定Sevlet的url、是否允許請求非同步(沒特別需求都是開true)
		...
		xmlhttp.onreadystatechange=function readyGo(){//收到東西就執行以下函式內容
		  if(xmlhttp.readyState==4 && xmlhttp.status==200){//確定都收到才執行以下貼上動作 先判斷網路連線狀態是否連線已經都已經完成 4是Complete代表已收到全部資料 200就是平常網頁打不開時顯示404、500的那個數字 正常會是200
			var alreadyRegister = xmlhttp.responseText;//收到的資料會存在xmlhttp物件裡面 .response可以得出來 確定是文字的話可用.responseText
